...
# -*- coding: utf-8 -*-
"""
Created on Sat May  1 14:00:33 2021

@author: kostas
"""

"""
 THE PRESENT CODE OPENS THE WRF FILES OF DOMAIN 02  AND THEN IT 
...
print ("\n (d02 Visualization) Initialization time = ",start_time)
# -----------------------------------------------------------------------------
...
filenames = [f for f in os.listdir(pwd_directory) if f.startswith('wrfout_d02')]
# Define the WRF GRID RESOLUTION for the current domain
wrf_grid_resolution =' Res:6km'   # !!! need to change that point
...
d02_basic_path="path_to_WRF_visualization/d02_temp_maps"  #!!!
...
# snow_colors = ["#ffffff","#f8faea", "#f1f6d6", "#eaf2c1", "#e3eead",
#                "#ddea98", "#d6e684", "#cfe26f", "#c8de5a", "#c1da46",
#                "#bbd632", "#a8c02d", "#95ab28", "#829523", "#70801e",
#                "#5d6b19", "#4a5514", "#38400f", "#252a0a"]
# snow_colormap = matplotlib.colors.ListedColormap(snow_colors)
...
for i in tqdm(range(12,len(time))): 
 ...
 ### PROBLEM WITH Interpolation for 500,700,850 hPa  pressure levels IN TEMPERATURE RESULTS 
      #### !!!! DONT USE THEM!!!!
...
for i in range (0,len(variable_directory)):
    files = glob.glob(d02_basic_path + variable_directory[i]+'/*')
    for f in files:
        os.remove(f) 
    

print("\n ------- Construction of wrfout maps for d02 domain  ----------\n")
...
for i in tqdm(range(12,len(time))):  
    ...
    os.chdir(d02_basic_path + wind_500_dir) # SOS:WE CHANGE DIRECTORY TO SAVE THE PLOT WHERE WE WANT        
    ...
    os.chdir(d02_basic_path + temp_500_dir) # SOS:WE CHANGE DIRECTORY TO SAVE THE PLOT WHERE WE WANT        
    ...
    os.chdir(d02_basic_path + wind_700_dir) # SOS:WE CHANGE DIRECTORY TO SAVE THE PLOT WHERE WE WANT        
    ...
    plt.barbs(to_np(lons[::8,::8]), to_np(lats[::8,::8]),
              to_np(u_700[::8, ::8]), to_np(v_700[::8, ::8]),
              transform=ccrs.PlateCarree(), length=6)
    ...
    os.chdir(d02_basic_path + temp_700_dir) # SOS:WE CHANGE DIRECTORY TO SAVE THE PLOT WHERE WE WANT        
    ...
    os.chdir(d02_basic_path + wind_850_dir) # SOS:WE CHANGE DIRECTORY TO SAVE THE PLOT WHERE WE WANT        
    ...
    os.chdir(d02_basic_path + temp_850_dir) # SOS:WE CHANGE DIRECTORY TO SAVE THE PLOT WHERE WE WANT        
    ...
    temp_850_levels = np.arange(temp_850_min-5,temp_850_max+5,1) # specify the step of contours
    ...
    os.chdir(d02_basic_path + cloud_fraction_all_clouds_dir) # SOS:WE CHANGE DIRECTORY TO SAVE THE PLOT WHERE WE WANT        
    ...
    os.chdir(d02_basic_path + wind_10_slp_dir) # SOS:WE CHANGE DIRECTORY TO SAVE THE PLOT WHERE WE WANT        
    ...
    plt.barbs(to_np(lons[::6,::6]), to_np(lats[::6,::6]),
              to_np(U10_wrf[i][::6, ::6]), to_np(V10_wrf[i][::6, ::6]),
              transform=ccrs.PlateCarree(), length=6)    
    ...
    os.chdir(d02_basic_path + t2m_dir) # SOS:WE CHANGE DIRECTORY TO SAVE THE PLOT WHERE WE WANT        
    ...
    lat =lat_made
    lon = lon_made
    wrf_temp_2m_levels = np.arange(wrf_temp_2m_min-1,wrf_temp_2m_max+3,2) # specify the step of contours
    ...
    # os.chdir(d02_basic_path + snow_dir) # SOS:WE CHANGE DIRECTORY TO SAVE THE PLOT WHERE WE WANT        
    ...
    os.chdir(d02_basic_path + rh2_dir) # SOS:WE CHANGE DIRECTORY TO SAVE THE PLOT WHERE WE WANT        
    ...
    os.chdir(d02_basic_path + rain_dir) # SOS:WE CHANGE DIRECTORY TO SAVE THE PLOT WHERE WE WANT   
...
wrf_snowh_levels = [1,2,3,4,5,8,10,15,20,25,30,35,40,45,50,60,70,80] 
for i in tqdm(range(12,len(time))):    
...
plt.close()
    ...
    # os.chdir(d02_basic_path + Max_refl) # SOS: WE CHANGE DIRECTORY TO SAVE THE PLOT WHERE WE WANT
...
print (" (d02) Visualitation:> Total execution time = ",dif)   
