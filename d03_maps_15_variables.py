...
# -*- coding: utf-8 -*-
"""
Created on Mon Apr 19 16:05:00 2021

@author: kostas
"""

"""
 THE PRESENT CODE OPENS THE WRF FILES OF DOMAIN 03  AND THEN IT 
...
ATTENTION: WHEN SOMEONE NEEDS TO USE THE PRESENT CODE ON HIS MACHINE HE HAS TO
           CHANGE THE !!! LINES WHICH ARE:
               d03_basic_path =...... (line 101) 
               HE MAY NEED TO CHANGE THE REST OF THE PATHS THERE BUT EVERETHING ELSE IS
               ALREADY SET.
               
"""

#!!!  FOR A SITE THE BEST SOLUTION IS dpi=72 FOR MAP RESOLUTION !!!!!!!!!!!!!!!!!
...
print ("\n (d03 Visualization) Initialization time = ",start_time)
# -----------------------------------------------------------------------------
...
filenames = [f for f in os.listdir(pwd_directory) if f.startswith('wrfout_d03')]
# Define the WRF GRID RESOLUTION for the current domain
wrf_grid_resolution =' Res:1,2km'   # !!! need to change that point
...
# d03_basic_path = "/MODELS/WRF-intel/archive/web/gifakia/python_maps/d03_temp_maps"

d03_basic_path="path_to_WRF_visualization/d03_temp_maps"  #!!!
...
for i in tqdm(range(12,len(time))): 
    ...
    # rain_max_list.append(((wrf_total_precip[i].max()).values) - (wrf_total_precip[i-1].max()).values)
...
for i in range (0,len(variable_directory)):
    files = glob.glob(d03_basic_path + variable_directory[i]+'/*')
    for f in files:
        os.remove(f) 
    

print("\n ------- Construction of wrfout maps for d03 domain  ----------\n")
...
for i in tqdm(range(12,len(time))):  
    ...
    os.chdir(d03_basic_path + wind_500_dir) # SOS:WE CHANGE DIRECTORY TO SAVE THE PLOT WHERE WE WANT        
    ...
    for w in range(0, len(cities)):
        ax.plot(city_lon[w], city_lat[w], 'bo',color='magenta', markersize=5, transform=ccrs.Geodetic())
    ...
    levels = np.arange(wind_500_min-1, wind_500_max+1, 2.)
    ...
    os.chdir(d03_basic_path + temp_500_dir) # SOS:WE CHANGE DIRECTORY TO SAVE THE PLOT WHERE WE WANT        
    ...
    os.chdir(d03_basic_path + wind_700_dir) # SOS:WE CHANGE DIRECTORY TO SAVE THE PLOT WHERE WE WANT        
    ...
    # Add the wind speed contours
    levels = np.arange(wind_700_min-1, wind_700_max+1, 2.)
    ...
    os.chdir(d03_basic_path + temp_700_dir) # SOS:WE CHANGE DIRECTORY TO SAVE THE PLOT WHERE WE WANT        
    ...
    # ax.gridlines()     
    # Titles
    plt.title('700 hPa Temperature (\u2103), Barbs (kt)\n\n\n ',size=15)
    plt.title('Valid: '+str(wrf_total_precip[i].Time.values)[:13]+ 'UTC \n', loc='left',size=15)
    plt.title('WRF-ARW (V4.1.5) '+wrf_grid_resolution+ '\n Init: '+filenames[0][:24]+'UTC \n', loc='right',size=12)
    # Save the plot
    ...
    os.chdir(d03_basic_path + wind_850_dir) # SOS:WE CHANGE DIRECTORY TO SAVE THE PLOT WHERE WE WANT        
    ...
    # levels = np.arange(z_850_min-1, z_850_max+1, 2.)
    ...
    # Add the wind speed contours
    levels = np.arange(wind_850_min-1, wind_850_max+1, 2.)
    ...
    # ax.gridlines() 
    # Titles
    plt.title('850 hPa Wind Speed (kt), Barbs (kt) \n\n\n ',size=15)
    plt.title('Valid: '+str(wrf_total_precip[i].Time.values)[:13]+ 'UTC \n', loc='left',size=15)
    plt.title('WRF-ARW (V4.1.5) '+wrf_grid_resolution+ '\n Init: '+filenames[0][:24]+'UTC \n', loc='right',size=12)     
    # Save the plot
    ...
    os.chdir(d03_basic_path + temp_850_dir) # SOS:WE CHANGE DIRECTORY TO SAVE THE PLOT WHERE WE WANT        
    ...
    plt.barbs(to_np(lons[::8,::8]), to_np(lats[::8,::8]),
              to_np(u_850[::8, ::8]), to_np(v_850[::8, ::8]),
              transform=ccrs.PlateCarree(), length=6)
 ...
 #%% (7,8,9) CLOUD FRACTION FOR ALL  -->(low,medium,high) clouds
    ...
    os.chdir(d03_basic_path + cloud_fraction_all_clouds_dir) # SOS:WE CHANGE DIRECTORY TO SAVE THE PLOT WHERE WE WANT        
    ...
    os.chdir(d03_basic_path + wind_10_slp_dir) # SOS:WE CHANGE DIRECTORY TO SAVE THE PLOT WHERE WE WANT        
    ...
    # Add the wind speed contours
    levels = np.arange(wind_10_slp_min, wind_10_slp_max+3, 1.)
    wrf_wspd__contour =ax.contour(to_np(lons), to_np(lats), to_np(wrf_WSpeed_10m),
                                  levels=levels,colors='k',linewidths=0.1,
                                  transform=ccrs.PlateCarree())
    plt.clabel(wrf_wspd__contour, inline=True, fmt='%1i', fontsize=8)
    ...
    # Add the 850 hPa wind barbs, only plotting every 2nd data point.
    plt.barbs(to_np(lons[::4,::4]), to_np(lats[::4,::4]),
              to_np(U10_wrf[i][::4, ::4]), to_np(V10_wrf[i][::4, ::4]),
              transform=ccrs.PlateCarree(), length=6)    
    ...
    # ax.gridlines() 
    # Titles
    plt.title('Wind Speed at 10m Height (kt),Wind Barbs (kt) \n\n\n ',size=15)
    plt.title('Valid: '+str(wrf_total_precip[i].Time.values)[:13]+ 'UTC \n', loc='left',size=15)
    plt.title('WRF-ARW (V4.1.5) '+wrf_grid_resolution+ '\n Init: '+filenames[0][:24]+'UTC \n', loc='right',size=12)
    # Save the plot   
    ...
    os.chdir(d03_basic_path + t2m_dir) # SOS:WE CHANGE DIRECTORY TO SAVE THE PLOT WHERE WE WANT   
    ...
    plt.savefig('Temperature_2m__'+filenames[0][:24]+'UTC'+'__Valid:_'+str(wrf_total_precip[i].Time.values)[:13]+'UTC__.png',
                dpi=150,bbox_inches='tight')
    plt.close()
    #%% (12)   """ PLOT THE SNOW HEIGHT """
    """ PLOT THE SNOW HEIGHT """
 
    # os.chdir(d03_basic_path + snow_dir) # SOS:WE CHANGE DIRECTORY TO SAVE THE PLOT WHERE WE WANT        
    ...
    plt.title('WRF-ARW (V4.1.5) '+wrf_grid_resolution+ '\n Init: '+filenames[0][:24]+'UTC \n', loc='right',size=12)
    # Save the plot  
    plt.savefig('Snow__'+filenames[0][:24]+'UTC'+'__Valid:_'+str(wrf_total_precip[i].Time.values)[:13]+'UTC__.png',
                dpi=150,bbox_inches='tight')
    plt.close()
    ...
    os.chdir(d03_basic_path + rh2_dir) # SOS:WE CHANGE DIRECTORY TO SAVE THE PLOT WHERE WE WANT        
    ...
    lat =lat_made
    lon = lon_made
    wrf_rh2_levels = np.arange(rh2_min,rh2_max+1,5) # specify the step of contours
    ...
    os.chdir(d03_basic_path + rain_dir) # SOS:WE CHANGE DIRECTORY TO SAVE THE PLOT WHERE WE WANT   
    ...
    lat =lat_made
    lon = lon_made 
    wrf_total_precip_levels = 1.28**np.arange(0, nn, 1)-1 # specify the step of contours
...
# snow_colors = ["#fdfdfd", "#c1da46", "#bbd632", "#b2d119", "#aacc00", "#92d119",
#                "#86cc00", "#41d119", "#2ccc00", "#00cc6d", "#00b762", "#00a357",
#                "#008e4c", "#007a41", "#006636", "#00512b", "#003d20", "#f69e1d",
#                "#dd8504", "#05f0d6", "#05b7f0", "#0565f0"]
# snow_colormap = matplotlib.colors.ListedColormap(snow_colors)
...
lat =lat_made
lon = lon_made 
# wrf_snowh_levels = np.arange(1,61,5) # specify the step of contours 
# nn = 18                     #number of steps
# wrf_snowh_levels = 1.29**np.arange(0, nn, 1)-1 # specify the step of contours

wrf_snowh_levels =[1,2,3,5,8,10,15,20,25,30,35,40,45,50,55,60,70] 

for i in tqdm(range(12,len(time))):    
    wrf_snowh_contour =ax.contour(to_np(lons), to_np(lats),wrf_snow_wat_equiv[i],
                                  cmap=snow_colormap,
                                levels=wrf_snowh_levels,linewidths=0.01, zorder=3, transform = ccrs.PlateCarree())
...
plt.close()
    ...
    # os.chdir(d03_basic_path + Max_refl_dir) # SOS: WE CHANGE DIRECTORY TO SAVE THE PLOT WHERE WE WANT
    ...
    # max_reflect_levels  = np.arange(Max_refl_max-1,Max_refl_max+3,2) # specify the step of contours
...
print (" (d03) Visualitation:> Total execution time = ",dif)
    
######################################################################

#%%
# Method to find the nearest grid point.!!


# def find_nearest_grid_point(wrf_file, time, lats, lons):
#     '''
#     Input:
#     era_data: the era5 netcdf file
#     time, lat, lon is in the format as follows
#         time: '2018-02-15T06:06:00'
#         lat: '-55.5555123123'
#         lon: '155.55512323'
#     Output: 
#     ERA5 profile at a grid point with 37 levels
#     '''
#     time=wrf_total_precip[i].Time.values
#     return wrf_file.t.sel(time=time,latitude=lats,longitude=lons,method='nearest')


# xxx = find_nearest_grid_point(wrf_file, str(wrf_total_precip[i].Time.values)[:16],
#                              str(city_lat[1]), str(city_lon[1]))
